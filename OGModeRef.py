...
for ff in range (0, NFREQs):
	...
	for j in range (0, Ny):
		for i in range (0, Nx):
			...
			# print((ff*Ny + j)*Nx + i)
			ExRef[i][j] = ExR[(ff*Ny + j)*Nx + i] + 1j*ExI[(ff*Ny + j)*Nx + i]
			EyRef[i][j] = EyR[(ff*Ny + j)*Nx + i] + 1j*EyI[(ff*Ny + j)*Nx + i]
			EzRef[i][j] = EzR[(ff*Ny + j)*Nx + i] + 1j*EzI[(ff*Ny + j)*Nx + i]

			ExTra[i][j] = ExTR[(ff*Ny + j)*Nx + i] + 1j*ExTI[(ff*Ny + j)*Nx + i]
			EyTra[i][j] = EyTR[(ff*Ny + j)*Nx + i] + 1j*EyTI[(ff*Ny + j)*Nx + i]
			EzTra[i][j] = EzTR[(ff*Ny + j)*Nx + i] + 1j*EzTI[(ff*Ny + j)*Nx + i]

			ExInc[i][j] = ExIncR[(ff*Ny + j)*Nx + i] + 1j*ExIncI[(ff*Ny + j)*Nx + i]
			EyInc[i][j] = EyIncR[(ff*Ny + j)*Nx + i] + 1j*EyIncI[(ff*Ny + j)*Nx + i]
			EzInc[i][j] = EzIncR[(ff*Ny + j)*Nx + i] + 1j*EzIncI[(ff*Ny + j)*Nx + i]
...
plt.savefig("PlaneWaveAgSiRTJuly31.pdf")
