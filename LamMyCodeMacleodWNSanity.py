...
d = 80e-9
imp0 = 376.730313
k0 = linspace (100000, 2000000, 20000)
c0 = 3e8
...
R = np.loadtxt("Ref.txt", usecols=(0), skiprows= 1000000, unpack =True )
...
plt.plot(lam*1e6, (abs(Rfft)/abs(Ifft))**2, label = r'$\rm R_{My \ GPU \ Code:  Correct \ \gamma}$', color = "red", linewidth = 3)
plt.plot(lam*1e6, (abs(Tfft)/abs(Ifft))**2, label = r'$\rm T_{My \ GPU \ Code: Correct \ \gamma}$', color = "black", linewidth = 3)
plt.plot(lam*1e6, (abs(Tfft)/abs(Ifft))**2 + (abs(Rfft)/abs(Ifft))**2, label = r'$\rm R+T_{My \ GPU \ Code: Correct \ \gamma}$', color = "limegreen", linewidth = 3)
...
plt.savefig("GPUMyCodehBNRealGammaMacConfirmJune29_2020LamShort.pdf")
plt.savefig("GPUMyCodehBNRealGammaMacConfirmJune29_2020LamShort.png")
