...
for k in range(0,28):
	...
	for ax0 in axs.flat:
		...
		for i in range (350, 750):
			Full[i-350] = Ez[i*Nx: i*Nx + Nx]# + Ey[i*Nx: i*Nx + Nx]+ Ez[i*Nx: i*Nx + Nx]
	...
	plt.savefig("EzMaps/PDF/Ez2.22XSecHM%.2f_%.2f_3_7Mag.pdf" %(base, lam))
	plt.savefig("EzMaps/PNG/Ez2.22XSecHM%.2f_%.2f_3_7Mag.png" %(base, lam))
#abs(Full*(dt/T))
