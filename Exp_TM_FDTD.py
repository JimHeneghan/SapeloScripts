...
##############################################################################
fig, (ax0, ax1, ax2)  = plt.subplots(3,figsize=(18,30), sharex = True)
...
ax5.set_xticks([])
ax5.set_yticks([])
ax5.set_ylabel('Reflectance', rotation='vertical', fontsize = '70', labelpad = 50)
###########################################################################
i = 0
j = 0
q = 0
...
for i in range(0,5):
	...
	ax0.set_ylim(0,1)
	ax0.tick_params(direction = 'in', width=5, size = 10, labelsize=30)   
	ax0.tick_params(which ='minor', direction = 'in', width=3, size = 5)
	ax0.yaxis.set_major_locator(MaxNLocator(nbins = 5,prune='lower'))

	ax0.plot(1/(lam*1e-4), Ref, label = r'$\rm R_{FDTD \ %s \ nm}$' %dirk[i], color = cols[i],
		linewidth = 8)
...
#################################################################
	ax1.plot(k00/100, Rm, label=r'$\rm R_{TM \ %s \ nm}$' %dirks[i], color=cols[i], linewidth = 8)
...
for z in range (0, 5):   
	F1 = "CaldwellDigi/hBN%dnmRef.txt" %dirks[z]
	k_data, R_exp = loadtxt(F1, usecols=(0,1,),  unpack = True)

	ax2.plot(k_data, R_exp, 'o',  markersize=10, markeredgecolor = "black", linewidth = 5, markerfacecolor = cols[z])
	plt.setp(ax2.spines.values(), linewidth=2)
	ax2.tick_params(direction = 'in', width=5, size = 10, labelsize=30)
	ax2.set_xlabel(r"$\rm Frequency \ (cm^{-1})$", fontsize = '70')
...
for i in range(0, 5):
    temp = mpatches.Patch(facecolor=cols[i], label = r'$ d \rm = %2.2f \ nm$' %dirks[i], edgecolor='black')
    patches.append(temp) 
leg = ax2.legend(handles = patches, ncol = 3, loc = 'lower center', frameon = True,fancybox = False, 
fontsize = 30, bbox_to_anchor=(-0.1, -0.45, 1.2, .175),mode="expand", borderaxespad=0.) #bbox_to_anchor=(1.05, 0.5),
leg.get_frame().set_edgecolor('black')

leg.get_frame().set_linewidth(4)
###############################################################################  

plt.savefig("dSweepR3Pane.png")
