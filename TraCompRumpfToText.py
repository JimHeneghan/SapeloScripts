...
for i in range(0,5):
	...
	ax0.set_ylabel("T", fontsize = '30')   
	ax0.set_xlabel(r"$\rm Frequency \ (cm^{-1})$", fontsize = '30')
	ax0.set_xlim(1200,1700)
	ax0.set_ylim(0,1)


	ax0.plot(1/(lam*1e-4), Tra, label = r'$\rm T_{FDTD \ %s \ nm}$' %dirk[i], color = cols[i],
		linewidth = 8)
...
#################################################################
	ax0.plot(k00/100, Tm, label=r'$\rm T_{TM \ %s \ nm}$' %dirks[i], color=litecols[i], linewidth = 4)

ax0.legend(loc='center right', fontsize='15', ncol = 2)
plt.savefig("TraSweep.png")
