...
##############################################################################
fig, ax0 = plt.subplots(2,3, figsize=(15,9),constrained_layout=True)
##############################################################################
...
for i in range(0,5):
	...
	plt.setp(ax0[j,q].spines.values(), linewidth=2)
	ax0[j,q].tick_params(direction = 'in', width=2, labelsize=20)
	ax0[j,q].set_ylabel("R", fontsize = '30')   
	ax0[j,q].set_xlabel(r"$\rm Frequency \ (cm^{-1})$", fontsize = '30')
	ax0[j,q].set_xlim(1200,1700)
	ax0[j,q].set_ylim(0,1)


	ax0[j,q].plot(1/(lam*1e-4), Ref, label = r'$\rm R_{FDTD \ %s \ nm}$' %dirk[i], color = cols[i],
		linewidth = 8)
	# ax0[j,q].plot(1/(lam*1e-4), Tra, label = r'$\rm T_{%s}$' %dirk[q], color = "blue", linewidth = 3)
	# ax0[j,q].plot(1/(lam*1e-4), Abs, label = r'$\rm A_{%s}$' %dirk[q], color = "limegreen", linewidth = 3)

	ax0[j,q].tick_params(left = False, bottom = False)   
	ax0[j,q].yaxis.set_major_locator(MaxNLocator(nbins = 5,prune='lower'))
...
#################################################################
	ax0[j,q].plot(k00/100, Rm, label=r'$\rm R_{TM \ %s \ nm}$' %dirks[i], color=litecols[i], linewidth = 4)

#################################################################

	F1 = "CaldwellDigi/hBN%dnmRef.txt" %dirks[i]
	k_data, R_exp = loadtxt(F1, usecols=(0,1,),  unpack = True)

	ax0[j,q].plot(k_data, R_exp, 'o', label=r'$\rm R_{Exp \ %s \ nm}$' %dirks[i], markersize=5,
	 markeredgecolor = "black", linewidth = 5, markerfacecolor = litecols[i])

	ax0[j,q].legend(loc='center right', fontsize='15')

	q = q+1 
	if (q>2):
		j = 1
		q = 0
plt.savefig("dSweep6Pane.png")
